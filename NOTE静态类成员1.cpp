...
06. 有一个确保静态(static)成员只被定义一次的好方法就是
    把它的定义(define)和 非内联(inline)类成员的定义(define) 放在同一个文件中.
07. Tip:static 成员的血量就像 globle 对象一样长，一直存在到程序结束.

08. 我们一般不在类主体(body)内初始化静态成员，但是我们有时候会提供一种 类内初始化器(in-class initializer)
