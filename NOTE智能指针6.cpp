...
04.   对于智能指针 shared_ptr 时,更加推荐使用 make_shared();函数,而少用 new 关键字.
      ...
      func_cal(shared_ptr<int>(x));//此时出现了智能指针,那么它的计数器就会加1,表示有多了
                                   //一个指向保存有 20 的内存.
                                   //那么一旦这个函数结束后呢？
      int i = *x; //危险！！因为这个内存位置已经被释放了！因为计数器递减为0,智能指针认为此处的内存已经没有用了。
...
05.   unique_ptr:
      ...
      unique_ptr<type_name,callable_obj_type> ps2(build_in_ptr,callable_obj);
      //此语法的解释:<>内,第一个参数是需要指向的指针的类型(与前面的type_name同义),在()内的第一个参数体现
      //            <>内,第二个参数是可调用对象的类型,在()内第二个参数体现,该参数用于替换unique_ptr默认使用delete释放内存的行为(与前方类似).
...
--------------------------------------------------------------------------------------------------------------------
07.   weak_ptr:
      //weak_ptr绑定在一个被shared_ptr管理的对象上,这种智能指针不会影响shared指针的计数器,当shared计数器为零,或者
      //其他原因,被指向的对象都会被释放,不管它有没有weak_ptr指向.存在感很weakly的指针.
      //weak_ptr不能直接的使用其所指向的对象,一般使用lock()函数配合 if语句进行安全的使用.
      weak_ptr<type_name> ps3;
      weak_ptr<type_name> ps3(ps);//假设此处ps是一个shared_ptr类型的智能指针.
