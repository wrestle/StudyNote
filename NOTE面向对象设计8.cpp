...
07.   虚函数 Virtual Function
      ...
      derived-to-base),也就是继承类的类型指针可以隐式转换为基类的类型指针.如果不相同,那么继承类
      中的同名函数会隐藏基类中的函数,造成动态绑定失效.
...
08.   抽象基类 Abstract Base Classes
      ...
      ba_clp->test();         //不行!!因为成员搜索机制在base类中找不到test这个成员
        ...
        7.和正常变量一样,类中的成员会隐藏其基类的同名变量,对于函数亦是,此时只能通过域作用符访问(::)
        8.函数调用的步骤,见英文版 Page.619.KEY CONCEPT(INMORTANT);
09.     我们需要析构函数正确的释放所分配的内存,那就应该让基类的析构函数是虚函数(virtual),如果一个
      虚析构函数只有空的函数体的时候,那么它就不需要拷贝构造函数和赋值运算符了.
10.   Derived-Class Copy Constructor
